...
float3 _AgXSlope;
float3 _AgXPower;
float _AgXSaturation;

// Minimal Agx implementation from https://iolite-engine.com/blog_posts/minimal_agx_implementation
real3 agxDefaultContrastApprox(real3 x) {
    real3 x2 = x * x;
    real3 x4 = x2 * x2;
    real3 x6 = x4 * x2;

    // 7th order approximation of AgX curve
    return - 17.86     * x6 * x
           + 78.01     * x6
           - 126.7     * x4 * x
           + 92.06     * x4
...
real3 agxEotf(real3 val) {
    ...
    val = pow(val, real3(2.2, 2.2, 2.2));

    return val;
}

real3 agxLook(real3 val) {
